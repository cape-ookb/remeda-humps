...
Renamed to `remeda-humps`. Rewritten in TypeScript and switched from lodash to [Remeda](https://remedajs.com).

- Source and tests are now TypeScript. Ships with type declarations.
- Dual ESM + CommonJS builds via `tsup`. The default export stays CommonJS-friendly (`require('remeda-humps')` returns the function).
- `createHumps` is now published at the `remeda-humps/createHumps` subpath instead of `lib/createHumps`.
- Key casing is produced by Remeda's `toCamelCase`/`toSnakeCase` instead of lodash. Output is equivalent for the common underscore/hyphen/space/UPPERCASE cases.
- Tooling moved to `tsup` (build), `vitest` (test), and flat-config `eslint` + `typescript-eslint` (lint).
